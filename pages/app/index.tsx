    ...
    return (
        <div className="h-full w-full  overflow-auto flex flex-col px-10 py-8">
            ...
            <div className="flex flex-col">
                {
                    games.map((g: any) => (
                        <div key={g.id} className="py-3 px-4 my-2 rounded-md shadow dark:bg-gray-800 bg-white ">
                            <span className="text-lg font-bold">{g.name}</span>
                            <span></span>
                            <p className="whitespace-pre-wrap opacity-50 text-xs">{g.description}</p>
                            <div>
                                {g.joined ? <a href={`/app/game/${g.id}`}
                                               className="btn-primary !py-2 !px-3 float-right">Open</a>
                                    : <button onClick={() => joinGame(g.id)}
                                              className="btn-primary !py-2 !px-3 float-right">Join</button>
                                }
                            </div>
                        </div>))
                }
